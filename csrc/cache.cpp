...
//
// Dimension mapping (CUDA -> SYCL nd_range<2>):
//   CUDA blockDim.x=8 (head axis)  -> SYCL local_range[0]=8
//   CUDA blockDim.y=BLOCK_Y_SIZE   -> SYCL local_range[1]=BLOCK_Y_SIZE
//   CUDA blockIdx.y (head blocks)  -> SYCL group_id[0]
//   CUDA blockIdx.x (token blocks) -> SYCL group_id[1]
...
class cp_gather_indexer_k_quant_cache_kernel {
 public:
  ...
  void operator()(sycl::nd_item<2> item) const {
    ...
    // local_id[0] -> head sub-thread (0..7), analogous to CUDA threadIdx.x
    // local_id[1] -> token sub-thread (0..BLOCK_Y_SIZE-1), like CUDA
    // threadIdx.y
    const int local_x = static_cast<int>(item.get_local_id(0));
    const int local_y = static_cast<int>(item.get_local_id(1));
    const int local_range_x = static_cast<int>(item.get_local_range(0));  // 8

    const int token_idx = static_cast<int>(item.get_global_id(1));
    const int head_idx = static_cast<int>(item.get_global_id(0)) * VEC_SIZE;

    // Phase 1: find which batch sequence this token belongs to.
    // Threads along local_x cooperate to scan cu_seq_lens in chunks of 8.
    for (int iter = 0; iter < (batch_size_ + local_range_x - 1) / local_range_x;
         ++iter) {
      int tid = iter * local_range_x + local_x;
      if (tid < batch_size_) {
        const int seq_start = cu_seq_lens_[tid];
        const int seq_end = cu_seq_lens_[tid + 1];
        if (token_idx >= seq_start && token_idx < seq_end) {
          batch_idx_acc_[local_y] = tid;
        }
      }
    }
    ...
    const int my_batch = batch_idx_acc_[local_y];
    ...
    // Scale copy: only local_x==0 writes (one scale per quant_block)
    if (local_x == 0) {
...
// The local work-group is (8, BLOCK_Y_SIZE):
//   dim 0 (size 8)           = head-axis  (like CUDA threadIdx.x)
//   dim 1 (size BLOCK_Y_SIZE) = token-axis (like CUDA threadIdx.y)
#define CALL_CP_GATHER_INDEXER_K_QUANT_CACHE(BLOCK_Y_SIZE)              \
  do {                                                                  \
    constexpr int _vec_size = 16;                                       \
    sycl::range<2> _global(                                             \
        ((head_dim + 8 * _vec_size - 1) / (8 * _vec_size)) * 8,         \
        ((num_tokens + (BLOCK_Y_SIZE) - 1) / (BLOCK_Y_SIZE)) *          \
            (BLOCK_Y_SIZE));                                            \
    sycl::range<2> _local(8, (BLOCK_Y_SIZE));                           \
    queue.submit([&](sycl::handler& cgh) {                              \
      sycl::local_accessor<int, 1> _batch_idx(                          \
          sycl::range<1>((BLOCK_Y_SIZE)), cgh);                         \
      cgh.parallel_for(                                                 \
          sycl::nd_range<2>(_global, _local),                           \
