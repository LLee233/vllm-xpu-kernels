...
class indexer_k_quant_and_cache_kernel {
 public:
  ...
  void operator()(const sycl::nd_item<2>& item_id) const {
    ...
    int64_t head_dim_idx = (group_x * item_id.get_local_range(0) * item_id.get_local_range(1) +
                       local_x  * item_id.get_local_range(1) + local_y)* VEC_SIZE;
    ...
    for (int i = 0; i < VEC_SIZE; i++) {
        k_vals[i] = static_cast<float>(k_[token_idx * head_dim_ + head_dim_idx + i]);
    }
    ...
    for (int i = 0; i < VEC_SIZE; i++) {
        at::Float8_e4m3fn fp8_val = static_cast<at::Float8_e4m3fn>(k_vals[i] / scale);
        kv_cache_[dst_offset + i] = *reinterpret_cast<uint8_t*>(&fp8_val);
    }

    if (local_y == 0) {
        const int64_t dst_scale_idx = block_idx * cache_block_size_ * cache_stride_ +
...
void indexer_k_quant_and_cache(
  ...
  constexpr int vec_size = 4;
  sycl::range<2> grid((head_dim + quant_block_size * vec_size - 1) /
                      (quant_block_size * vec_size), num_tokens);
